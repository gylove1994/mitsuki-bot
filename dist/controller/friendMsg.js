...
exports.friendMsg = void 0;
const permissions_1 = require("./../middleware/permissions");
function friendMsg(context, mitsuki) {
    return __awaiter(this, void 0, void 0, function* () {
        if (context.msg.type == "FriendMessage") {
            ...
            throw new Error("friendMsg控制器错误的处理了非FriendMessage事件");
    });
}
exports.friendMsg = friendMsg;
...
function defaultMsg(context, mitsuki) {
    if (context.msg.type == "FriendMessage" && !mitsuki.getEnvValue("disableDefault").disableSet.includes(context.msg.sender.id)) {
        let endTime = 0;
        let time = "";
        if (mitsuki.getEnvValue("offTime").isSet) {
            ...
            context.msg.reply("现在这个QQ目前是由bot-mitsuki接管哦！\n接管结束时间还剩：" + time + "分钟，输入mitsuki -help获取可用命令列表");
        }
        else
            context.msg.reply("现在这个QQ目前是由bot-mitsuki接管哦！\n那个傻逼GY目前没有设置解除接管时间，输入mitsuki -help获取可用命令列表");
        ...
        setTimeout(() => {
            if (context.msg.type == "FriendMessage") {
                let disableSet = mitsuki.getEnvValue("disableDefault").disableSet.filter((id) => { return context.msg.type == "FriendMessage" && (context.msg.sender.id !== id); });
                mitsuki.setEnvValue("disableDefault", { disableSet: disableSet });
            }
        }, 10000);
    }
...
function ec(context, mitsuki) {
    if (context.msg.type == "FriendMessage") {
        ...
        mitsuki.api.sendFriendMessage("【紧急信息】" + context.msg.sender.nickname + ":" + context.getMiddlewareOutput("splitText").str[2], mitsuki.setting.qq_bind);
    }
}
