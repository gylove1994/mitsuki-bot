...
function defaultMsg<T extends MsgType>(context:Context<T>,mitsuki:Mitsuki){
    if(context.msg.type == "FriendMessage" && !mitsuki.getEnvValue<DisableDefault>("disableDefault").disableSet.includes(context.msg.sender.id)){
        let endTime = 0
        let time = ""
        ...
        setTimeout(()=>{
            if(context.msg.type == "FriendMessage"){
                let disableSet = mitsuki.getEnvValue<DisableDefault>("disableDefault").disableSet.filter((id)=>{return context.msg.type == "FriendMessage" && (context.msg.sender.id !== id)})
                mitsuki.setEnvValue("disableDefault",{disableSet:disableSet})
            }
        },10000)
    }
...
function ec<T extends MsgType>(context:Context<T>,mitsuki:Mitsuki){
    if(context.msg.type == "FriendMessage"){
        ...
        mitsuki.api.sendFriendMessage("【紧急信息】"+context.msg.sender.nickname+":"+context.getMiddlewareOutput<SplitText>("splitText").str[2],mitsuki.setting.qq_bind)
    }
}